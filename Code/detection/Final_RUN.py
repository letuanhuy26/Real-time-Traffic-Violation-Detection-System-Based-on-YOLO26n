        ...
        for box, tid, cls_id in zip(
            ...
            state = vehicle_memory.setdefault(
                tid,
                {
                    ...
                    "prev_box": None,
                    "prev_rear_y": None,
                    "prev_rear_x": None,  #  Lưu X trước
                    "already_crossed": False,
                    "crossed_vehical": set()
                }
            )

            # ================== CROSSING LOGIC (IMPROVED) ==================
            crossed = False
                ...
                if state["prev_rear_y"] is not None:
                    ...
                    if vehical_id not in state["crossed_vehical"]:
                        for sl in current_stoplines:
                            ...
                            # VI PHẠM: Chấm đỏ (rear_y) nằm trong vùng [center, top]
            ...
            if state["violated"] and not state["saved"]:
                ...
                if plate_crop is not None:
                    # Nếu detect được plate thì có confidence
                    plate_info = detect_license_plate(crop)
                    if plate_info:
                        plate_conf = plate_info["conf"]
                    
                    # ═══ CLAHE LẦN 1 trên plate crop ═══
                    ...
                    plate_clahe_2 = apply_clahe_on_plate(plate_clahe_1)
            ...
            # ═══════════════════════════════════════════════════════════
            # Store crop for sidebar display (if violation)
            # ═══════════════════════════════════════════════════════════
            if state["violated"]:
